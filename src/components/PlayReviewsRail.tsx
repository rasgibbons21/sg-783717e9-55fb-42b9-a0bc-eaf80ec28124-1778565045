    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-bold text-[#F4F7FA]">Reviews</h3>
        </div>
        <p className="text-xs text-[#F4F7FA]/40 mb-3">
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-base">⭐</span>
          <h3 className="text-sm font-bold text-[#F4F7FA]">Popular on Play</h3>
        </div>
        <span className="text-[10px] text-[#F4F7FA]/30">{reviews.length} reviews</span>
      </div>
        ...
        {reviews.slice(0, 5).map((review, i) => (
          <div key={i} className="space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-[#F4F7FA]/70">{review.name}</span>
              <span className="text-[10px] text-[#F4F7FA]/25">{review.date}</span>
            </div>
            <Stars count={review.rating} />
            <p className="text-xs text-[#F4F7FA]/50 line-clamp-2 leading-relaxed">{review.text}</p>
          </div>
        ))}
      </div>

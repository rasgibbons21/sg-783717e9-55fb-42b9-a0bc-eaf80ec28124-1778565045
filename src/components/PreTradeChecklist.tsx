...
function inputCls(hasValue: boolean) {
  return `w-full bg-background border ${hasValue ? "border-accent/40" : "border-accent/15"} rounded-lg px-3 py-2.5 text-foreground text-sm focus:outline-none focus:border-accent transition-colors`;
}
...
function chipCls(selected: boolean) {
  return `px-3 py-1.5 rounded-full text-xs border transition-colors cursor-pointer select-none ${
    selected
      ...
      : "bg-background border-accent/15 text-foreground/50 hover:border-accent/40 hover:text-foreground/80"
  }`;
}
...
function MiniChart({ ticker, priceLines }: { ticker: string; priceLines?: { entry?: number; stop?: number; target?: number } }) {
  ...
  return (
    <div className="rounded-xl border border-accent/20 bg-background overflow-hidden mb-4">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-2 px-3 py-2 text-xs text-accent/80 hover:text-accent transition-colors"
      >
        <BarChart2 className="w-3.5 h-3.5 flex-shrink-0" />
        <span className="font-mono font-semibold">{ticker}</span>
        <span className="text-foreground/30 ml-1">· Daily candles</span>
        <span className="ml-auto text-foreground/25">{open ? "▲" : "▼"}</span>
      </button>
      {open && (
        <div className="border-t border-accent/10">
          {loading ? (
            <div className="flex items-center justify-center h-[220px] text-accent/40 text-xs">
              Loading chart…
            </div>
          ) : bars.length > 1 ? (
            <DynamicChart data={bars} height={220} priceLines={priceLines} />
          ) : (
            <div className="flex items-center justify-center h-[220px] text-foreground/20 text-xs">
  ...
  return (
    <>
      ...
      <div className="space-y-3">
        <div>
          <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">Ticker</label>
          <input
        ...
        <div>
          <label className="text-xs text-foreground/40 mb-2 block uppercase tracking-wide">Direction</label>
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 py-3 rounded-xl border text-sm font-semibold transition-colors ${
                  f.direction === d
                    ...
                    : "bg-background border-accent/15 text-foreground/40"
                }`}
              >
...
function StepThesis({ f, set }: { f: Form; set: (k: keyof Form, v: string) => void }) {
  return (
    <>
      ...
      <div>
        <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">Your reason for this trade</label>
        <textarea
          ...
          autoFocus
        />
        <p className="text-[10px] text-foreground/25 mt-1.5">Be specific. Vague entries are the first warning sign.</p>
      </div>
    </>
  );
}
...
function StepTrend({ f, set }: { f: Form; set: (k: keyof Form, v: string) => void }) {
  return (
    <>
      ...
      <div>
        <label className="text-xs text-foreground/40 mb-2 block uppercase tracking-wide">Trend you&apos;re trading</label>
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-colors ${
                f.trend === t
                  ...
                  : "bg-background border-accent/10 text-foreground/50 hover:border-accent/30 hover:text-foreground/70"
              }`}
            >
              {t}
            </button>
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        <div>
          <label className="text-xs text-foreground/40 mb-2 block uppercase tracking-wide">Chart pattern</label>
        ...
        <div>
          <label className="text-xs text-foreground/40 mb-2 block uppercase tracking-wide">Candlestick confirmation</label>
          <div className="flex flex-wrap gap-2">
            {CANDLESTICKS.map(c => (
        ...
        <div>
          <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">Indicator confirmation (optional)</label>
          <input
...
function StepLevels({ f, set }: { f: Form; set: (k: keyof Form, v: string) => void }) {
  return (
    <>
      ...
      <div className="space-y-3">
        <div>
          <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">Support level (price)</label>
          <input
          ...
          />
          <p className="text-[10px] text-foreground/25 mt-1">Where buyers have stepped in before — your potential stop zone</p>
        </div>
        <div>
          <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">Resistance level (price)</label>
          <input
            type="number" step="any" min="0"
          ...
          />
          <p className="text-[10px] text-foreground/25 mt-1">Where sellers have pushed back before — your potential target zone</p>
        </div>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">Shares *</label>
            <input
          ...
          <div>
            <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">Entry price *</label>
            <input
              type="number" step="any" min="0.0001"
        ...
        <div>
          <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">
        ...
        <div>
          <label className="text-xs text-foreground/40 mb-1.5 block uppercase tracking-wide">Take-profit target</label>
          <input
            type="number" step="any" min="0"
        ...
        {rr && (
          <div className={`rounded-xl px-4 py-3 border ${
            rr.ratio >= 1.5 ? "bg-primary/8 border-primary/30" :
            rr.ratio >= 1 ? "bg-yellow-500/8 border-yellow-500/30" :
            "bg-destructive/8 border-destructive/30"
          }`}>
            <div className="flex items-center justify-between">
              <span className="text-xs text-foreground/50">Risk / Reward</span>
              <span className={`text-lg font-mono font-bold ${
                rr.ratio >= 1.5 ? "text-primary" : rr.ratio >= 1 ? "text-yellow-400" : "text-destructive"
              }`}>
                1 : {rr.ratio.toFixed(2)}
              </span>
            </div>
            <div className="flex gap-4 mt-1 text-[10px] text-foreground/35">
        ...
        {cost != null && !isNaN(cost) && (
          <p className="text-xs text-accent/70 bg-accent/8 rounded-lg px-3 py-2">
        ...
        {/* Warnings */}
        {noStop && (
          <div className="flex gap-2 items-start rounded-lg bg-destructive/8 border border-destructive/20 px-3 py-2.5">
  ...
  return (
    <>
      ...
      <div className="space-y-3 text-sm">
        <div className="rounded-xl bg-background border border-accent/15 divide-y divide-accent/10">
          {[
            ...
            <div key={label as string} className="flex gap-3 px-4 py-2.5">
              <span className="text-foreground/35 text-xs w-28 flex-shrink-0 pt-0.5">{label}</span>
              <span className="text-foreground/80 text-xs leading-relaxed">{value}</span>
            </div>
          ))}
        </div>
        {f.thesis && (
          <div className="rounded-xl bg-background border border-accent/15 px-4 py-3">
            <p className="text-[10px] text-foreground/30 uppercase tracking-wide mb-1.5">Your thesis</p>
            <p className="text-xs text-foreground/70 leading-relaxed">{f.thesis}</p>
          </div>
        )}
        ...
        {!f.stopPrice && (
          <div className="flex gap-2 items-start rounded-lg bg-destructive/8 border border-destructive/20 px-3 py-2.5">
            ...
            <p className="text-xs text-destructive/80">No invalidation level — you&apos;re entering without a stop.</p>
          </div>
        )}
        {rr && rr.ratio < 1 && (
          <div className="flex gap-2 items-start rounded-lg bg-destructive/8 border border-destructive/20 px-3 py-2.5">
            <AlertTriangle className="w-3.5 h-3.5 text-destructive flex-shrink-0 mt-0.5" />
            <p className="text-xs text-destructive/80">Risk/reward is below 1:1 — risking more than you stand to gain.</p>
          </div>
        )}
      </div>
    </>
...
export function PreTradeChecklist({ onSubmit, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-card rounded-2xl border border-accent/25 flex flex-col max-h-[90vh]">

        {/* Header */}
        <div className="flex items-center gap-3 px-5 pt-5 pb-4 border-b border-accent/10 flex-shrink-0">
          <div className="flex-1">
            <p className="text-[10px] text-accent/60 uppercase tracking-widest mb-0.5">
            ...
            {STEPS.map((_, i) => (
              <div
                ...
                className={`rounded-full transition-all ${
                  i === step ? "w-4 h-2 bg-accent" :
                  i < step ? "w-2 h-2 bg-primary" :
                  "w-2 h-2 bg-accent/20"
                }`}
              />
            ))}
          </div>
          <button onClick={onClose} className="text-foreground/30 hover:text-foreground ml-1">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 min-h-0">
          {renderStep()}
          {err && (
            <div className="mt-3 rounded-lg bg-destructive/10 border border-destructive/20 px-3 py-2.5 text-xs text-destructive">
              {err}
            </div>
          )}
        </div>
        ...
        {/* Footer */}
        <div className="px-5 pb-5 pt-4 border-t border-accent/10 flex gap-3 flex-shrink-0">
          {step > 0 && (
            <button
              type="button"
              onClick={handleBack}
              className="flex items-center gap-1.5 px-4 py-3 rounded-xl border border-accent/20 text-foreground/50 text-sm hover:text-foreground hover:border-accent/40 transition-colors"
            >
          ...
          {!isLast ? (
            ...
            <button
              ...
              disabled={submitting}
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary/90 disabled:opacity-50 transition-colors"
            >
              {submitting ? (
                "Opening…"
              ) : (

...
export async function sendWelcomeEmail(to: string, name: string) {
  ...
  return resend.emails.send({
    from: FROM,
    to,
    subject: "Welcome to Radar — let's get started",
    html: `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:520px;margin:0 auto;padding:32px 20px;color:#C8C2B8;background:#07080C;border-radius:12px">
        ...
        <p style="line-height:1.6;margin:0 0 16px">Your <strong style="color:#27B7C8">Radar account</strong> is ready. Full access to:</p>
        ...
        <p style="line-height:1.6;margin:0 0 24px">Explore everything Radar has to offer. Dive in!</p>
...
// Kept for backward compatibility — trials no longer exist.
export async function sendTrialExpiringEmail(_to: string, _name: string) {
  return null;
}

      ...
      <div className="bg-[#07080C] min-h-screen">
        <div className="max-w-3xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-2xl font-serif font-bold text-[#F3EDE3]">My Strategy Toolbelt</h1>
              <p className="text-xs text-[#F3EDE3]/40">{items.length} strategies saved</p>
            </div>
          </div>

          {items.length === 0 ? (
            <div className="text-center py-16">
              <Wrench className="w-12 h-12 text-[#F3EDE3]/10 mx-auto mb-4" />
              <p className="text-[#F3EDE3]/40 text-sm mb-2">Your toolbelt is empty.</p>
              <p className="text-[#F3EDE3]/25 text-xs mb-6">Add strategies as you study them to build your personal reference.</p>
            ...
            <div className="space-y-4">
              {items.map((item) => {
                ...
                return (
                  <div key={item.strategy_slug} className="bg-[#121821] rounded-xl border border-white/5 p-4">
                    <div className="flex items-start justify-between mb-2">
                      <Link href={`/university/strategy-lab/${strategy.slug}`} className="flex items-center gap-2 group">
                        <span className="text-xl">{strategy.icon}</span>
                        <div>
                          <h3 className="text-sm font-semibold text-[#F3EDE3] group-hover:text-[#27B7C8] transition-colors">
                            {strategy.name}
                          </h3>
                          <p className="text-[10px] text-[#F3EDE3]/30">{strategy.category} · {strategy.difficulty}</p>
                        </div>
                        <ChevronRight className="w-3 h-3 text-[#F3EDE3]/20" />
                      </Link>
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="p-1.5 text-[#F3EDE3]/30 hover:text-[#27B7C8] transition-colors"
                        >
                          <Edit3 className="w-3.5 h-3.5" />
                        </button>
                        <button
                          onClick={() => removeItem(item.strategy_slug)}
                          className="p-1.5 text-[#F3EDE3]/30 hover:text-red-400 transition-colors"
                        >
                    ...
                    {/* Toolbelt summary */}
                    <div className="text-[10px] text-[#F3EDE3]/40 space-y-0.5 mb-2">
                    ...
                    {!isEditing && item.personal_notes && (
                      <p className="text-xs text-[#F3EDE3]/50 italic mt-2 border-t border-white/5 pt-2">
                        {item.personal_notes}
                      </p>
                    )}
                    ...
                    {isEditing && (
                      <div className="mt-3 space-y-2 border-t border-white/5 pt-3">
                        <div>
                          <label className="text-[10px] text-[#F3EDE3]/40 block mb-1">Personal Notes</label>
                          <textarea
                            value={editNotes}
                            onChange={(e) => setEditNotes(e.target.value)}
                            placeholder="What did you learn? How will you use this?"
                            className="w-full bg-[#07080C] border border-white/10 rounded-lg px-3 py-2 text-xs text-[#F3EDE3]/70 placeholder:text-[#F3EDE3]/20 resize-none"
                            rows={3}
                          />
                        </div>
                        <div>
                          <label className="text-[10px] text-[#F3EDE3]/40 block mb-1">Biggest Mistake to Avoid</label>
                          <input
                            value={editMistake}
                            onChange={(e) => setEditMistake(e.target.value)}
                            placeholder="What's the #1 trap to watch out for?"
                            className="w-full bg-[#07080C] border border-white/10 rounded-lg px-3 py-2 text-xs text-[#F3EDE3]/70 placeholder:text-[#F3EDE3]/20"
                          />
                        </div>
                        <button

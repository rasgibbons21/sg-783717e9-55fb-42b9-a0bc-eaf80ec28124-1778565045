...
import { LESSONS, getLessonBySlug, getAdjacentLessons, Lesson } from "@/data/lessons";
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  return { props: { lesson, prev: prev ?? null, next: next ?? null } };
};

export default function LessonPage({ lesson, prev, next }: Props) {
  return (
    <>
      <Head>
      ...
      <SEO title={`${lesson.title} — She Blooms Wealth`} description={lesson.subtitle} />
      ...
      <div style={{ fontFamily: "'DM Sans', sans-serif", background: C.deep, color: C.ivory, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "1.5rem 1.5rem 0", width: "100%" }}>
          <Link href="/learn" style={{ fontSize: 13, color: "rgba(244,247,250,0.4)", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}>
            ← All Lessons
          </Link>
        </div>
        ...
        <main style={{ flex: 1, maxWidth: 720, margin: "0 auto", padding: "2rem 1.5rem 5rem", width: "100%" }}>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: "1.5rem", flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 11, color: "rgba(244,247,250,0.3)" }}>·</span>
            <span style={{ fontSize: 11, color: "rgba(244,247,250,0.4)" }}>{lesson.readTime} min read</span>
            <span style={{ fontSize: 11, color: "rgba(244,247,250,0.3)" }}>·</span>
            <span style={{ fontSize: 11, color: "rgba(244,247,250,0.4)" }}>{lesson.difficulty}</span>
          </div>
          ...
          {/* Body */}
          <div style={{ fontSize: "1rem", lineHeight: 1.78, color: "rgba(244,247,250,0.78)" }}>
            {lesson.body.split("\n\n").map((para, i) => (
          ...
          <div style={{ display: "flex", gap: 16, marginTop: "2rem", flexWrap: "wrap" }}>
            {prev && (
              <Link href={`/learn/${prev.slug}`} style={{ flex: 1, minWidth: 200, textDecoration: "none" }}>
            ...
            {next && (
              <Link href={`/learn/${next.slug}`} style={{ flex: 1, minWidth: 200, textDecoration: "none", marginLeft: "auto" }}>
